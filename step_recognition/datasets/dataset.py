        ...
        for vid in self.vids:
            ...
            if self.training:
                ...
                for start, end in zip(
                    range(seed, target.shape[0], self.stride),
        ...
        for vid in self.vids:
            ...
            if self.training:
                ...
                for start, end in zip(
                    range(seed, target.shape[0], self.stride),
                    range(
                        seed + self.window_size,
                        target.shape[0] - self.anticipation_length,
                        self.stride,
                    ),
                ):
                    self.inputs.append(
                        [
                            ...
                            target[start:end],
                            target[end : end + self.anticipation_length],
                        ]
                    )
            else:
                start = 0
...
class FINEACTIONDataset(data.Dataset):
        ...
        for vid in self.vids:
            ...
            if self.training:
                ...
                for start, end in zip(
                    ...
                    self.inputs.append([vid, start, end])
            else:
                start = 0
                end = target.shape[0]
                self.inputs.append([vid, start, end])
    ...
    def __getitem__(self, index):
        ...
        rgb_input = np.load(
            osp.join(self.root_path, self.rgb_type, vid + ".npy"), mmap_mode="r"
        )[start:end]
        flow_input = np.load(
            osp.join(self.root_path, self.flow_type, vid + ".npy"), mmap_mode="r"
        )[start:end]
        target = np.load(
            osp.join(self.root_path, self.annotation_type, vid + ".npy"), mmap_mode="r"
        )[start:end]
        rgb_input = torch.tensor(rgb_input.astype(np.float32))
        flow_input = torch.tensor(flow_input.astype(np.float32))
        target = torch.tensor(target.astype(np.float32))
